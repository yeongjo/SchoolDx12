...
int CMesh::GetIntersectRayCount(XMFLOAT3& xmf3RayOrigin, XMFLOAT3& xmf3RayDirection, float *pfNearHitDistance) {
	return false;
	...
	//if (m_pnSubSetIndices > 0) nPrimitives = (m_d3dPrimitiveTopology ==
	//	D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST) ? (m_pnSubSetIndices / 3) : (m_pnSubSetIndices - 2);
	//광선은 모델 좌표계로 표현된다. 
	...
	//		XMVECTOR v0 = XMLoadFloat3((XMFLOAT3 *)(pbPositions + ((m_pnSubSetIndices) ?
	//			(m_pnIndices[(i*nOffset) + 0]) : ((i*nOffset) + 0)) * m_nStride));
	//		XMVECTOR v1 = XMLoadFloat3((XMFLOAT3 *)(pbPositions + ((m_pnSubSetIndices) ?
	//			(m_pnIndices[(i*nOffset) + 1]) : ((i*nOffset) + 1)) * m_nStride));
	//		XMVECTOR v2 = XMLoadFloat3((XMFLOAT3 *)(pbPositions + ((m_pnSubSetIndices) ?
...
/////////////////////////////////////////////////////////////////////////////////////
//
CGeometryBillboardMesh::CGeometryBillboardMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void *pVertices, UINT nVertices, UINT nStride) : CMesh(pd3dDevice, pd3dCommandList) {
	m_nVertices = nVertices;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;
	m_pd3dPositionBuffer = CreateBufferResource(pd3dDevice, pd3dCommandList, pVertices, nStride * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dPositionUploadBuffer);
...
CGeometryBillboardMesh::~CGeometryBillboardMesh() = default;
