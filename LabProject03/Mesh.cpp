...
float CHeightMapImage::GetHeight(float fx, float fz) {
	...
	fx = mod(fx, (float)m_nWidth);
	fz = mod(fz, (float)m_nLength);
...
CCubeMeshIlluminated::CCubeMeshIlluminated(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, float fWidth, float fHeight, float fDepth) : CMeshIlluminated(pd3dDevice, pd3dCommandList) {
	...
	m_pd3dVertexBuffer = CreateBufferResource(pd3dDevice, pd3dCommandList, pxmf3Positions, sizeof(XMFLOAT3) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dVertexUploadBuffer);
