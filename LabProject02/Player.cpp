...
//이 함수는 매 프레임마다 프레임워크에서 호출된다. 플레이어의 속도 벡터에 중력과 마찰력 등을 적용
void CPlayer::Update(float fTimeElapsed){
	...
	위 벡터에 마찰력을 곱하여 감속 벡터를 구한다. 속도 벡터에 감속 벡터를 더하여 속도 벡터를 줄인다. 마찰력이 속력보다 크면 속력은 0이 될 것이다.*/
...
void CPlayer::Shot(){
	...
	for(size_t i = 0; i<bullets.size(); i++){
		if(!bullets[i]->m_bActive){
			bullets[i]->SetPosition(m_xmf3Position);
			bullets[i]->SetMovingDirection(m_xmf3Look);
			bullets[i]->m_bActive = true;
			bullets[i]->target = target;
			return;
		}
	}

	auto t = new CBullet(m_xmf3Look);
	t->SetMesh(&CCubeMesh::getInstance());
	t->SetPosition(m_xmf3Position);
	t->target = target;
	bullets.push_back(t);
}

void CPlayer::SetTarget(float x, float y){
	XMFLOAT3 start(x,y,0);
	start = CGraphicsPipeline::Screen2ViewPort(start);
	XMFLOAT3 end(start.x, start.y, 1);
	CPlayer::OnUpdateTransform();
	CGraphicsPipeline::SetWorldTransform(&m_xmf4x4World);
	start = CGraphicsPipeline::Viewport2World(start);
	end = CGraphicsPipeline::Viewport2World(end);
	auto direc = XMVector3Normalize(XMLoadFloat3(&end)-XMLoadFloat3(&start));
	//bool Intersects(XMVECTOR origin, direction, float &distance);
	XMStoreFloat3(&end, direc);
	target = CGameFramework::getInstance().CheckCollision(start, end);
	print(L"settarget:%d", target);
}
