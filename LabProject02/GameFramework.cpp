...
CGameObject * CGameFramework::CheckCollision(const CGameObject * rhs){
	return m_pScene->CheckCollision(rhs);
}

CGameObject * CGameFramework::CheckCollision(XMFLOAT3 origin, XMFLOAT3 direc){
	return m_pScene->CheckCollision(origin, direc);
}
...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT
	...
	case WM_LBUTTONDOWN:
		...
		if(m_pPlayer) m_pPlayer->SetTarget(LOWORD(lParam), HIWORD(lParam));
		break;
		//마우스 캡쳐를 해제한다. 
	case WM_LBUTTONUP:
	case WM_RBUTTONUP:
